def pandas_describe(df):
	...
	:WHAT IT DOES: This provides mean, stddev, min and max in a pandas dataframe on any data.
...
def sum_describe(df):
	...
	* df = the dataframe that you are calling this on.
	...
	:EXAMPLE:
			for file in files:  
	>>>      df = sum_describe(df = df)
	"""
		import pyspark.sql.functions as F
		out = df.groupBy().sum()
		out = (out
					 .toDF(*[x.replace('(',')').split(')')[1] for x in out.columns]))
		out = out.toPandas().transpose().reset_index()
...
def positive_describe(df):
	...
	:WHAT IT DOES: This looks at the positive values in the dataset and counts them by column.
...
def negative_describe(df):
	...
	:WHAT IT DOES: This looks at the negative values in the dataset and counts them by column.
...
def zero_describe(df):
	...
	:WHAT IT DOES: This looks at the false values in the dataset and counts them by column.
...
def null_describe(df):
	...
	:WHAT IT DOES: This looks at the null values in the dataset and counts them by column.
...
def unique_describe(df):
	...
	:WHAT IT DOES: This looks at the unique values in the dataset and counts them by column.
	:RETURNS: Pandas dataframe with a numerical sum showing negative.
	:OUTPUT VARIABLE TYPE: Out 1: Information on the data you have in the dataset as a pandas dataframe.
	:TESTED TO RUN ON: test data in adruk.test.QA
...
def stddev_describe(df):
	...
	:WHAT IT DOES: Tihs looks at the std numerical value and provides this in a pandas dataframe.
...
def min_describe(df):
	...
	:WHAT IT DOES: This looks at the minimum values in the dataset and counts them by column.
...
def mode_describe(df):
	...
	:WHAT IT DOES: This looks at the boolean values and provides information on the positive boolean figure.
	...
	* df = the dataframe that you are calling this on.
	...
	:EXAMPLE:
			df = mode_describe(df = df) 
	"""
		import pyspark.sql.functions as F
		from functools import reduce
		out = [(df.groupBy(column).count()
...
def special_describe(df,regex_dict):
	...
	* df = the dataframe that you are calling this on.
	...
	:EXAMPLE:
			df = special_describe(df = df) 
	"""
		import pyspark.sql.functions as F
		out_dict = {}
		for k,v in regex_dict.items():
			out = (df.select([F.count(F.when(df[c]
								.rlike(v), True)).alias(c) for c in df.columns]))
			out = out.toPandas().transpose().reset_index()
...
def extended_describe(
  ...
  :PARAMETERS:
	* df = spark dataframe to describe
	* all = chooses that all are true and overwrites calling subfunctions.
	* trim = this imports the trim function to be used a sub functon.
	* Active_columns = 
	* Sum = The function sum_describe gets called in here.
