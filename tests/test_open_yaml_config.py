...
class TestOpenYamlConfig(object):
  ...
  def test_open_file_not_exist(self):
    with pytest.raises(OSError):
      adr.open_yaml_config(self.yaml_non_existing_file_path)



  #2 Does the function raise an error when the yaml file has syntax errors?
  def test_open_yaml_syntax_error(self):
    with pytest.raises(yaml.parser.ParserError):
      adr.open_yaml_config(self.yaml_errors_file_path)



  #3. Does the function return a dictionary?
  def test_return_open_yaml_config(self):
    assert isinstance(adr.open_yaml_config(self.yaml_file_path),dict)



  #4. Is the dictionary empty (=empty yaml file)?
  def test_dictionary_open_yaml_config(self):
    assert len(adr.open_yaml_config(self.yaml_file_path)) != 0
  ...
  def test_values_open_yaml_config(self):
    assert (adr.open_yaml_config(self.yaml_file_path)["settings"]["file"]== "test/file/test.csv")
    assert (adr.open_yaml_config(self.yaml_file_path)["settings"]["columns"]== ["column_1","column_2","column_3"])
    assert (adr.open_yaml_config(self.yaml_file_path)["settings"]["test_data"]== True)
    assert (adr.open_yaml_config(self.yaml_file_path)["settings"]["number_of_columns"]== 3)
